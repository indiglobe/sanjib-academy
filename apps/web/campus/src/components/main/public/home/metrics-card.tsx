...
export function MetricsCard({ ...props }: {} & ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn(
        `bg-primary-50 text-primary-500 dark:text-foreground border-primary-500 m-auto flex h-full w-full max-w-75 flex-col items-center justify-start gap-y-4 rounded-lg border px-4 py-10 text-center`,
        props.className,
      )}
    />
  );
